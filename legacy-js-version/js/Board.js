var HOSTNAME = "localhost:3000"; // "stockservice.azurewebsites.net"
...
class Board {
  constructor(
    ...
    this.strategyDescription = {
      "Averaging Down":
        "Sells stocks when price increases (by UB); When price drops (by LB), purchases more (by T), and so on [UB1,LB1,T1,UB2,LB2,T2,...UB]",
      "Averaging Down Lazy":
        "Hold stocks until price drops (by LB) and purchase more (by T) and sell all when price increases (by UB) [LB, #, LB, #, ....., UB]",
  ...
  updateStrategyTitle() {
    ...
    var title =
      this.strategyDescription[selectedStrategy] == undefined
        ? ""
  ...
  */
  ...
  addSymbolToList(symbol) {
    $.ajax({
      type: "PUT",
      url: `http://${HOSTNAME}/addstock?symbol=` + symbol,
      context: this,
      success: function (data) {
        if (data == "success") {
  ...
  removeSymbolFromList(symbol) {
    $.ajax({
      type: "PUT",
      url: `http://${HOSTNAME}/deletestock?symbol=` + symbol,
      context: this,
      success: function (data) {
        if (data == "success") {
  ...
  GetDefaultSymbolList() {
    return new Promise((resolve, reject) => {
      $.ajax({
        type: "GET",
        url: `http://${HOSTNAME}/getdefaultlist`,
        dataType: "json",
        tryCount: 0,
        retryLimit: 3,
        success: function (data) {
  ...
  selectSummaryPanel($summary) {
    if (
      this.$selectedSummary == null ||
      $summary[0].id != this.$selectedSummary[0].id
    ) {
  ...
  */
  ...
  reset() {
    ...
    this.clearSummaryPanel(this.$summary4);
    this.clearStockChart();
  }
  ...
  */
  ...
  displayHistoricalDataAndStockChart(
    ...
    if (this.stockCache[selectedStock]) {
      ...
      $.ajax({
        type: "GET",
        url: `http://${HOSTNAME}/` + selectedStock,
        dataType: "text",
        context: this,
        success: function (data) {
